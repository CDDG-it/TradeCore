...
export default function NewTradePage() {
  ...
  function calcVisibleScore(d: TradeDiscipline, removed: string[]): number {
    const active = DISCIPLINE_LABELS.filter((item) => !removed.includes(item.key));
    const builtinPassed = active.filter((item) => d[item.key as keyof TradeDiscipline] as boolean).length;
