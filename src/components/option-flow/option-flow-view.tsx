...
// ── Page ──────────────────────────────────────────────────────────────────────
/**
 * Option Flow — dealer positioning, session structure and COT-driven weekly
 * bias for NQ & GC.
 *
 * Rendered in two places: as a subtab of Global Markets (`embedded`, where the
 * page already supplies a title and sub-nav) and, for anyone landing on the old
 * /option-flow link, as a redirect into that subtab. In embedded mode the page
 * header is dropped and the live/refresh controls move inline, so the section
 * does not repeat the heading above it.
 */
export function OptionFlowView({ embedded = false }: { embedded?: boolean }) {
  ...
  // Freshness readout + manual refresh. Sits in the page header on the standalone
  // route, and inline above the content when embedded as a Global Markets subtab.
  const liveControls = (
    ...
    <PageWrapper>
      {embedded ? (
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs text-muted-foreground">
            Dealer positioning, session structure and COT-driven weekly bias for NQ &amp; GC.
          </p>
          {liveControls}
        </div>
      ) : (
