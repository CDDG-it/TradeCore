            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              {rows.map((inst) => {
                ...
                return (
                  ...
                  >
                    <span className={`font-mono text-[11px] font-bold tracking-[0.1em] ${on ? "text-primary" : "text-foreground/85"}`}>
                      {inst.symbol}
                    </span>
          ...
          {!selected ? (
            ...
            <div className="space-y-4">
                ...
                {[
                  ...
                  return (
                    <div key={k} className="flex items-center gap-2">
                      <span className="w-[76px] shrink-0 text-[10px] text-muted-foreground/70">{k}</span>
              ...
              <div className="border-l-2 pl-2.5" style={{ borderColor: SIGNAL_TONE[selected.signal.kind] }}>
                <p className="text-[11px] font-semibold" style={{ color: SIGNAL_TONE[selected.signal.kind] }}>
                  {selected.signal.label}
                </p>
                <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground/80">{selected.signal.detail}</p>
              </div>
            </div>
          )}
        </Pane>
