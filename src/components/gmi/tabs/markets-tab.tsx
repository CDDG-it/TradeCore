    ...
    <div className="flex h-full min-h-0 flex-col gap-2">
            ...
            {sel && (
              <div className="pointer-events-none absolute left-3 top-3 w-[188px] border border-border/50 bg-background/80 p-3 backdrop-blur-md">
                ...
                <div className="mt-2 flex items-baseline gap-1.5">
                  ...
                  <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground/50">% · 10Y</span>
                </div>
                <div className="mt-2.5 space-y-[3px] border-t border-border/40 pt-2">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground/45">Δ 1m</span>
                    <span className="font-mono text-[10px] tabular-nums" style={{ color: toneFor(sel.value != null && sel.prev != null ? sel.value - sel.prev : null) }}>
                      {bp(sel.value != null && sel.prev != null ? sel.value - sel.prev : null)}
                    </span>
                  </div>
                  {sel.id !== "us" && (
                    <div className="flex items-baseline justify-between gap-2">
                      <span className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground/45">vs US</span>
                  ...
                  {sel.fxSymbol && q.get(sel.fxSymbol) && (
                    <div className="flex items-baseline justify-between gap-2">
                      <span className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground/45">{sel.fxSymbol}</span>
                      <Delta value={q.get(sel.fxSymbol)!.changePct} />
                    </div>
                  )}
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground/45">Bank</span>
        ...
        >
          {yields.length === 0 ? (
            ...
            <div className="flex h-full min-h-0 flex-col">
              <div className="grid shrink-0 grid-cols-[2.2rem_1fr_3.4rem_3.2rem_3.2rem] gap-x-2 border-b border-border/30 px-3 py-1.5 font-mono text-[8px] uppercase tracking-[0.16em] text-muted-foreground/40">
                <span />
                <span>Economy</span>
                ...
                {rows.map(({ y, change, spread }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="relative text-right font-mono text-[12px] font-semibold tabular-nums text-foreground">
                        {y.value != null ? y.value.toFixed(2) : "—"}
                      </span>
                      <span className="relative text-right font-mono text-[10px] tabular-nums" style={{ color: toneFor(change) }}>
                        {bp(change)}
                      </span>
                      <span className="relative text-right font-mono text-[10px] tabular-nums text-muted-foreground/70">
                        {y.id === "us" ? "—" : bp(spread)}
                      </span>
                    </button>
                  );
      ...
      <div className="grid shrink-0 grid-cols-1 gap-2 lg:h-[236px] lg:grid-cols-12">
        ...
        <Pane index="05" label="Dollar & volatility" right={<Meta env={quotesEnv} />} scroll className="lg:col-span-3">
          ...
          <p className="mt-2 border-t border-border/30 pt-1.5 font-mono text-[8px] uppercase leading-relaxed tracking-[0.14em] text-muted-foreground/35">
